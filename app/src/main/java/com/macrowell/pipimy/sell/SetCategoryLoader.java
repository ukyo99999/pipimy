...
public class SetCategoryLoader extends
        AsyncTaskLoader<List<CategoriesBean>> {
    ...
    public SetCategoryLoader(Context context) {
        super(context);
    }
    ...
    public List<CategoriesBean> loadInBackground() {
        ...
        try {
            ...
            for (int i = 1; i < jsonDataArray.length(); i++) { //i從一開始是因為要把喜愛商品項目濾掉
